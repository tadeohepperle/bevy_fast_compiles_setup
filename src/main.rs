...
fn setup(mut commands: Commands, asset_server: Res<AssetServer>) {
    let font = asset_server.load(r"fonts\BalooBhai2-Bold.ttf");
...
fn animate_translation(
    ...
    for mut transform in &mut query {
        transform.translation.x = 100.0 * time.elapsed_seconds().sin() * 2.0 - 400.0;
        transform.translation.y = 100.0 * time.elapsed_seconds().tan();
    }
}
...
fn animate_scale(
    ...
    for mut transform in &mut query {
        ...
        transform.scale = Vec3::splat((time.elapsed_seconds().tan() + 1.1) * 0.5);
    }
}
